...
const WearableConnection = ({ onBack, onNext }) => {
  ...
  return (
    <>
      ...
      <div className="flex flex-col h-screen overflow-hidden relative" style={{ background: '#FFFBF5', color: '#2D2416' }}>
      {/* Ambient Background */}
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[600px] h-[600px] rounded-full blur-[120px]" style={{ background: 'rgba(245, 158, 11, 0.15)' }}></div>
        <div className="absolute bottom-0 right-0 w-[400px] h-[400px] rounded-full blur-[100px]" style={{ background: 'rgba(245, 158, 11, 0.08)' }}></div>
      </div>
      ...
      <header className="relative z-10 flex items-center px-6 py-6 bg-transparent shrink-0">
        <button
          onClick={onBack}
          className="flex items-center justify-center size-10 rounded-full bg-white/60 backdrop-blur-sm border border-white/40 text-gray-600 hover:text-gray-900 hover:bg-white/80 transition-colors"
        >
          <span className="material-symbols-outlined" style={{ fontSize: '24px' }}>
            arrow_back
          </span>
        </button>
        <h1 className="text-xl font-bold tracking-tight text-gray-800 ml-4">Connect Your Devices</h1>
        <div className="ml-auto">
          <div className="size-10 rounded-full bg-[#F59E0B]/10 border border-[#F59E0B]/30 flex items-center justify-center text-[#F59E0B]">
            <span className="material-symbols-outlined" style={{ fontSize: '24px' }}>
              watch
            </span>
          </div>
      ...
      <main className="relative z-10 flex-1 flex flex-col w-full px-6 min-h-0 overflow-y-auto no-scrollbar pb-32">
          ...
          {devices.map((device) => (
            <div
              ...
              className={`rounded-2xl p-4 flex flex-col items-center gap-3 relative group overflow-hidden transition-all duration-300 ${
                device.isOther
                  ? 'glass-card cursor-pointer border-dashed border-gray-300/50 hover:border-gray-400/60'
                  : 'glass-card cursor-pointer hover:bg-white/80'
              } ${isDeviceConnected(device.id) ? 'border-[#F59E0B]/60 shadow-[0_0_20px_rgba(245,158,11,0.25)]' : ''}`}
              style={{ background: 'rgba(255, 255, 255, 0.7)', backdropFilter: 'blur(12px)', border: isDeviceConnected(device.id) ? '1px solid rgba(245, 158, 11, 0.6)' : '1px solid rgba(200, 190, 175, 0.4)' }}
            >
              {!device.isOther && isDeviceConnected(device.id) && (
                <>
                  <div className="absolute inset-0 bg-gradient-to-b from-[#F59E0B]/8 to-transparent"></div>
                  <div className="absolute top-2 right-2 text-[#F59E0B]">
              ...
              {device.isOther && (
                <span className="material-symbols-outlined text-gray-500" style={{ fontSize: '32px' }}>
                  {device.icon}
                </span>
              )}
              {!device.isOther && (
                <div className="text-center relative z-10">
                  ...
                  <p className="text-[10px] text-gray-500 mt-0.5">{device.description}</p>
                </div>
              )}
              {device.isOther && (
                <h3 className="font-medium text-gray-500 text-xs">{device.name}</h3>
              )}
              {!device.isOther && (
                <button
        ...
        <div className="animate-float">
          <div
            ...
            style={{ background: 'rgba(255, 255, 255, 0.7)', backdropFilter: 'blur(12px)', border: '1px solid rgba(200, 190, 175, 0.4)' }}
          >
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#F59E0B]/8 rounded-full blur-2xl -mr-10 -mt-10"></div>
            ...
            <div className="flex items-center justify-between mb-4 relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-gray-800">Data Sync Preview</h3>
                  <p className="text-[10px] text-gray-500">Importing from Oura Ring</p>
                </div>
              </div>
              <div className="px-2 py-1 bg-amber-500/10 rounded border border-amber-500/30 text-[10px] font-bold text-amber-600">
                Active
              </div>
            </div>
            ...
            <div className="space-y-3 relative z-10">
              {dataSyncItems.map((item, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between p-2 rounded-lg bg-gray-100/60 border border-gray-200/50"
                >
                  <div className="flex items-center gap-3">
                  ...
                  <span className="material-symbols-outlined text-[#00BFA5]" style={{ fontSize: '18px' }}>
                    check
                  </span>
                </div>
              ))}
      ...
      {/* Bottom CTA Button */}
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-[#FFFBF5] via-[#FFFBF5]/95 to-transparent z-50">
        <button
          onClick={onNext}
          className="w-full h-14 bg-[#F59E0B] text-white border border-[#F59E0B] rounded-xl flex items-center justify-center gap-2 font-bold text-base hover:bg-[#F59E0B]/90 transition-colors active:scale-95 duration-200 shadow-[0_0_20px_rgba(245,158,11,0.3)]"
        >
          Finish Setup
